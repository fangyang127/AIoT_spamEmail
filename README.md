...
- 使用 feature branches（`feat/<short-desc>`），開 PR 指向 `main`（或視專案流程使用 `dev`）
- 提交訊息建議遵循 Conventional Commits
- 若修改 capability 或新增功能，請新增 `openspec/changes/<id>/proposal.md` 並在 CI 中執行 openspec 驗證

## 聯絡與後續

- 需要我幫忙的項目範例：
	- 將 README 翻成英文並加入螢幕截圖
	- 把 Dashboard 部署到 Streamlit Cloud（包含 models 的處理）
	- 將 Token 預計算儲存到訓練 artifact 以加快 UI 啟動

## 授權

- 預設未指定授權。如果需要，我可以幫你加入 MIT / Apache-2.0 授權檔（`LICENSE`）並提交修改。

---

如果你想要我在 README 中加入：
- 實際的模型超參數（我可以把 `src/train.py` 的設定列出）
- 範例輸出截圖（我可以生成並加到 repo）
- 英文版的 README
